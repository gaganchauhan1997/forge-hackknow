    ...
    <Link
      ...
      className={`flex items-center gap-3 px-4 py-2.5 text-sm font-medium transition-colors ${
        isActive(to)
          ...
          : "text-[#1a1a1a]/80 hover:bg-[#F9FF00]/20"
      }`}
    >
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-y-auto py-2">
          ...
          return (
            <div key={cat.id}>
              <button
                ...
                className="w-full flex items-center gap-2 px-4 py-2.5 mt-1 hover:bg-[#F9FF00]/10 transition-colors"
              >
        ...
        <div className="mt-4 border-t border-[#1a1a1a]/10 pt-2">
      ...
      {loadUser().plan === "free" && (
        <div className="p-4 m-3 bg-[#F9FF00]/20 border-2 border-[#F9FF00]">
          <div className="flex items-center gap-2 mb-1">
            <Crown size={16} className="text-[#F9FF00]" />
            <span className="font-oswald text-xs font-bold uppercase">Upgrade to Premium</span>
          </div>
...
export default function AppLayout() {
  ...
  return (
    <div className="min-h-screen bg-[#faf6e9] flex flex-col">
      {/* Top Bar */}
      <header className="sticky top-0 z-50 bg-white border-b-2 border-[#1a1a1a]/10 flex items-center h-14 px-4 gap-3">
        {/* Logo */}
        <Link to="/app" className="flex items-center gap-2 shrink-0">
          <div className="w-7 h-7 bg-[#F9FF00] flex items-center justify-center font-oswald font-black text-sm text-[#1a1a1a]">
            F
          </div>
          <span className="hidden md:block font-oswald text-sm font-bold uppercase tracking-wider">
            FORGE YAHAVI
          </span>
        </Link>
        ...
        <button
          ...
          className="hidden md:flex w-7 h-7 items-center justify-center border border-[#1a1a1a]/20 hover:bg-[#F9FF00]/20 transition-colors"
        >
          <ChevronLeft size={14} />
        </button>
        ...
        {/* Search */}
        <div className="flex-1 max-w-lg mx-auto hidden md:flex items-center gap-2 bg-[#f5f5f5] px-3 py-1.5 border border-[#1a1a1a]/10">
          <Search size={14} className="text-[#1a1a1a]/40" />
          <input
            type="text"
            placeholder="Search tools, resumes, keywords..."
            className="flex-1 bg-transparent text-sm font-inter outline-none placeholder:text-[#1a1a1a]/40"
          />
          <kbd className="text-[10px] text-[#1a1a1a]/30 font-mono border border-[#1a1a1a]/10 px-1">⌘K</kbd>
        </div>
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          {/* Bell */}
          <button className="relative w-8 h-8 flex items-center justify-center hover:bg-[#F9FF00]/20 transition-colors">
            <Bell size={18} />
          ...
          {/* User Avatar */}
          {user.loggedIn ? (
            <Link to="/app/settings" className="w-8 h-8 bg-[#1a1a1a] text-white flex items-center justify-center font-oswald text-sm font-bold" style={{ borderRadius: "50%" }}>
              {(user.name || "U")[0].toUpperCase()}
            </Link>
          ) : (
            <Link to="/app/login" className="font-oswald text-xs font-bold uppercase hover:text-[#FF0004] transition-colors">
              LOG IN
            </Link>
          )}
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Desktop Sidebar */}
        <aside
          className={`hidden md:flex flex-col bg-white border-r-2 border-[#1a1a1a]/10 transition-all duration-200 shrink-0 ${
        ...
        {sidebarOpen && (
          <>
            ...
            />
            <div className="md:hidden fixed inset-y-0 left-0 w-[280px] bg-white z-50 flex flex-col shadow-2xl">
              <div className="flex items-center justify-between px-4 py-3 border-b-2 border-[#1a1a1a]/10">
                <div className="flex items-center gap-2">
                  <div className="w-7 h-7 bg-[#F9FF00] flex items-center justify-center font-oswald font-black text-sm">
                    F
                  </div>
      ...
      {/* Mobile Bottom Nav */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t-2 border-[#1a1a1a]/10 z-30 flex items-center justify-around h-16 px-2">
...
function BottomNavItem({
  ...
  if (center) {
    return (
      <Link to={to} className="flex flex-col items-center gap-0.5 -mt-4">
        ...
        } shadow-lg`} style={{ borderRadius: "50%" }}>
          {icon}
        </div>
        <span className={`text-[9px] font-oswald uppercase tracking-wide ${active ? "text-[#F9FF00]" : "text-[#1a1a1a]/50"}`}>
          {label}
        </span>
      </Link>
    );
  ...
  return (
    <Link to={to} className="flex flex-col items-center gap-0.5 py-1 px-2">
      <span className={active ? "text-[#F9FF00]" : "text-[#1a1a1a]/40"}>{icon}</span>
      <span className={`text-[9px] font-oswald uppercase tracking-wide ${active ? "text-[#1a1a1a] font-bold" : "text-[#1a1a1a]/50"}`}>
        {label}
      </span>
    </Link>
  );
