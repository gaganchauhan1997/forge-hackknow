...
const DEMO_RESUMES = [
  ...
  { id: "4", title: "Marketing Manager Resume", company: "Marketing", icon: "▶", atsScore: 70, editedAt: "5 days ago", color: "#333", content: "" },
];
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 pb-24 md:pb-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-oswald text-2xl font-bold text-white">My Resumes</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {resumes.map((resume) => (
          <div key={resume.id} className="bg-[#2a2a2a] border border-white/10 p-4 hover:border-[#F9FF00] transition-colors">
            ...
            <h3 className="font-oswald text-sm font-bold text-white">{resume.title}</h3>
            ...
            <div className="flex items-center gap-2 mt-3 pt-3 border-t border-white/5">
              <div
                className="w-8 h-8 flex items-center justify-center text-[10px] font-bold text-[#F9FF00]"
                style={{
                  borderRadius: "50%",
                  backgroundColor:
                    resume.atsScore >= 80 ? "rgba(249,255,0,0.1)" : resume.atsScore >= 60 ? "rgba(255,165,0,0.1)" : "rgba(255,0,4,0.1)",
                }}
              >
                {resume.atsScore}
              </div>
        ...
        <Link
          to="/app/tool/builder"
          className="border border-dashed border-white/20 p-8 flex flex-col items-center justify-center text-center hover:border-[#F9FF00] transition-colors min-h-[180px]"
        >
          <Plus size={24} className="text-white/20 mb-2" />
          <span className="font-oswald text-xs font-bold uppercase text-white/30">
            Create New Resume
          </span>
        </Link>
      </div>
